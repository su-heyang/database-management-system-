...
public class Create {
        ...
        for (String[] field : fields) {
            ...
            if (field[1].equals("int")) {
                ...
                try {
                    ...
                    throw new MyException("char长度参数出错，请输入数字");
                }
            } else {
                // 数据类型错误，抛出异常
                throw new MyException("无法处理的数据类型（仅支持int,char,double）");
            }
    ...
    public DBFContent executeSQL() throws MyException {
        // 读取类中的创建表的数据，在磁盘中创建表，表名为文件名
        File file = new File(DBMS.DATA_PATH_TABLE + tableName + ".dbf");
        if (file.exists()) {
            // 表已存在，抛出异常
            throw new MyException("表已存在,请重新建表！或删除已有表！（不推荐）");
        }
...
//        System.out.println(content.getFields());
    ...
    private void inputContraint() {
        File file = new File(DBMS.DATA_PATH_TABLE + "constraint.dbf");
        if (!file.exists()) {//文件不存在，创建，表中有tableName,fieldName,PrimaryKey,Unique,NotNull五个属性
            System.out.println("////////////////////////");
            DBFContent content = new DBFContent();
            content.setFieldCount(5);//字段数为属性值
            content.setRecordCount(fields.size());//记录数为列数
